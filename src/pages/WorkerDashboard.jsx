...
function OfferCard({ offer, expanded, onToggle, onAccept, onDecline }) {
  ...
  return (
    <article className={`rounded-xl border border-l-4 p-4 transition-shadow
      ${isPending
    ...
    >
      <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
        <div>
          <p className="text-sm font-semibold text-gray-700 leading-snug">{offer.description}</p>
          <p className="text-xs text-gray-400 mt-0.5">From: {offer.residentName}</p>
        </div>
        <div className="flex items-center gap-2">
          <StatusBadge status={offer.status} />
          <button onClick={onToggle} className="text-xs text-[#2e75b6] font-semibold hover:underline flex items-center gap-1">
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-gray-200 grid grid-cols-2 gap-2 text-xs text-gray-600 mb-3">
          <span className="flex items-center gap-1.5"><MapPin size={12} className="text-gray-400" />{offer.location}</span>
          <span className="flex items-center gap-1.5"><CalendarDays size={12} className="text-gray-400" />{offer.preferredDate}</span>
          <span className="flex items-center gap-1.5"><Banknote size={12} className="text-gray-400" />Budget: ₱{offer.budgetMin} – ₱{offer.budgetMax}</span>
        </div>
      )}
      {isPending && (
        <div className="flex justify-end gap-2 mt-2">
          <button onClick={onDecline} className="flex items-center gap-1.5 px-4 py-1.5 border border-red-300 text-red-600 text-xs font-semibold rounded-lg hover:bg-red-50 transition-colors">
            <XCircle size={14} />Decline
          </button>
          <button onClick={onAccept} className="flex items-center gap-1.5 px-4 py-1.5 bg-[#1d9e75] text-white text-xs font-semibold rounded-lg hover:bg-[#0f6e56] transition-colors">
